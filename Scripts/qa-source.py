...
if 'ROZZA 4.2.2 · REMOTE BG STABLE' not in html: errors.append('wrong visible version label')
...
if 'Ignored WebKit startup interruption' not in dj: errors.append('real-device WebKit startup interruption classifier missing')
...
if 'suspendForInterruption' not in dj or 'resumeAfterInterruption' not in dj: errors.append('native interruption path is not transport-only')
proj=(root/'project.yml').read_text()
for token in ['MARKETING_VERSION: 4.2.2','CURRENT_PROJECT_VERSION: 32','Resources/yt_background_bridge.js','path: rozza2.html']:
