...
if 'ROZZA 4.2.0 · CORE STABLE' not in html: errors.append('wrong visible version label')
...
if 'skipForwardCommand.isEnabled = true' not in dj or 'skipBackwardCommand.isEnabled = true' not in dj: errors.append('accessory skip commands missing')
...
for token in ['MARKETING_VERSION: 4.2.0','CURRENT_PROJECT_VERSION: 30','Resources/yt_background_bridge.js','path: rozza2.html']:
...
if errors:
    ...
    sys.exit(1)
print(f'ROZZA CORE-STABLE source QA PASS — {len(p.ids)} ids, no duplicates, artwork cover + signature + remote guards present.')
